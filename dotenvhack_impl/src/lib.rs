...
pub fn dotenv(input: TokenStream) -> TokenStream {
    if cfg!(debug_assertions) {
      if let Err(err) = dotenv::from_filename(".dev.env") {
        panic!("Error loading .dev.env file: {}", err);
      }
    } else {
      if let Err(_) = dotenv::from_filename(".prod.env") {
        if let Err(err) = dotenv::from_filename(".env") {
          panic!("Error loading .env & .prod.env file: {}", err);
        }
      }
    }
    // * 수정한 버전, .env, .prod.env, .dev.env (debug / test) 검사 하도록함
